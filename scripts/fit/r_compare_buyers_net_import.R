##  GENERATE PLOTS FOR COMPARISON OF SIMULATED AND REAL FOOD DATA: IMPORT-EXPORT QUANTITY   ##
...
for(i in seq(1,nrow(data.food))){
	country<-country+1
	filename<-paste("Plots/ABM/net import ",ordered.zones[i],".jpg",sep="")
	jpeg(filename)
	#country.food.fao<-as.numeric(food.fao[i,])
	#country.food.population<-as.numeric(food.population[i,])
	maxInPlot<-max(c(as.numeric(fao.net.imports[country,]),as.numeric(net.imports.from.sim.yearly[country,1:length(years)])))
	minInPlot<-min(c(as.numeric(fao.net.imports[country,]),as.numeric(net.imports.from.sim.yearly[country,1:length(years)])))
	plot(years,fao.net.imports[country,],type="l",main=paste(ordered.zones[i],"net imports \n FAO (black) simulations (red)"),ylim=c(minInPlot,maxInPlot))
	lines(years,net.imports.from.sim.yearly[country,1:length(years)],col=2)
