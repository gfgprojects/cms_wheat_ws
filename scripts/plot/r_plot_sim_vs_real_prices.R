...
#USA reservation price
data.producers<-read.csv("../../output/z_producers.csv")
sim.reservationPrice<-data.producers$ReservationPrice[which(data.producers$Name=="United States of America")]
...
start.plot.pos.sim<-which(round((time.sim.year-0.5))==years[1])
...
sim.reservation.prices.monthly_1<-data.frame(time=time.sim.months,price=sim.reservationPrice)
...
a<-ggplot(data=toplot)+theme_bw()+coord_cartesian(xlim=c(1993,2013),ylim=c(1,5))+labs(title="Weighted world price",subtitle="\\hskip4mm $\\spadesuit$ simulation \\hskip4mm \\color{green!55!black} \\huge \\raisebox{-0.8mm}{$\\bullet$} \\normalsize \\color{black} real",y="normalized annual price")+theme(panel.border = element_rect(color="black",size=1),panel.grid.major=element_line(colour="black",linetype="dotted"),plot.title = element_text(hjust = 0.5))
...
c<-ggplot(data=sim.prices.monthly_1)+theme_bw()+coord_cartesian(xlim=c(1993,2013),ylim=c(0.8,10))+labs(y="normalized monthly price")+theme(panel.border = element_rect(color="black",size=1),panel.grid.major=element_line(colour="black",linetype="dotted"))
c<-c+geom_line(aes(x=time,y=price))
c<-c+geom_line(data=usa.wheat.data,aes(x=time,y=wheat),col="green4")
c<-c+geom_line(data=sim.reservation.prices.monthly_1,aes(x=time,y=price),col="blue")
...
c<-c+annotate("text",x=1993,y=9,label="$\\bullet$",colour="green4",size=5,hjust=0)+annotate("text",x=1994,y=9,label="real USA, monthly",colour="green4",size=4,hjust=0)
c<-c+annotate("text",x=1993,y=9.5,label="$\\spadesuit$",size=3,hjust=0)+annotate("text",x=1994,y=9.5,label="simulation USA, monthly",size=4,hjust=0)
